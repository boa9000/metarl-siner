...
class Agent():
    ...
    def run_inner(self, env, is_training=True, is_meta_training=True):
        ...
        year = np.random.randint(1999,2001)
        period = (day, month, year, day, month+1, year)
        timestepph = 1
        extra_params={'timesteps_per_hour' : timestepph, 'runperiod': period}
        env = gym.make(env, config_params = extra_params)
        ...
        for episode in range(num_of_iterations + lost_episodes):
            ...
            while (not done):
                ...
                if info["time_elapsed(hours)"] % 24 == 0:
                    done = True
                    #print(info["time_elapsed(hours)"])
                else:
    ...
    def update_outer_reptile(self, meta_model, models):
        for name, param in meta_model.state_dict().items():
            param = param.clone()
            for env in models:
                param += self.lr_outer * (models[env].state_dict()[f'meta_model.{name}'] - param)
            meta_model.state_dict()[name] = param
        ...
        if is_meta_training:
            for iteration in range(self.outer_iterations):
                ...
                if self.meta_algorithm == 'reptile':
                    self.update_outer_reptile(self.meta_model, models)
                elif self.meta_algorithm == 'maml':
                    self.update_outer_maml(self.meta_model, models)
            ...
            rewards_dict_meta = {env: None for env in self.env_ids}
            rewards_dict_metaless = {env: None for env in self.env_ids}
            self.meta_model.load_state_dict(torch.load(self.MODEL_FILE, weights_only=True))
    ...
    def save_meta_graph(self, rewards_with_model, rewards_without_model):
        fig, axs = plt.subplots(len(self.env_ids), 1, figsize=(12, 6 * len(self.env_ids)))
